...
function Sidebar() {
  ...
  useEffect(() => {
    ...
    else if (state.pathname.includes('/dashboard')) setClick(1)
    else if (state.pathname.includes('/products')) setClick(2)
    else if (state.pathname.includes('/users')) setClick(3)
  }, [state])
  ...
  return (
    !sidebarHidden && <div className={style.Sidebar}>
      ...
      <div onClick={handleLogout} className={style.logout}>
        <svg height={'28px'} xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
