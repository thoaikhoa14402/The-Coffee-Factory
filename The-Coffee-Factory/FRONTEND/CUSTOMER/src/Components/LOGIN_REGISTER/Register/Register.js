...
function Register(props) {
  ...
  const handleRegister = (event) => {
    ...
    axios.post(process.env.REACT_APP_ipAddress + '/tcf/v1/users/signup', object).then((res) => {
      setStep(2)
    }).catch(error => {
      ...
      <div className='Register'>
        {step === 1 && <form onSubmit={handleRegister} className='step1'>
          ...
          <div className='createAccount'>
            ...
            <p style={{ color: 'red', fontWeight: '700' }} onClick={() => navigate('/login')}>Đăng nhập</p>
          </div>
        ...
        {step === 2 && <div className='step2'>
          ...
          <div className='registerSuccess'>
            ...
            <button style={{ marginTop: '-5px' }} onClick={() => navigate('/login')} className='registerButton'>đăng nhập ngay</button>
          </div>
