...
exports.History_User = catchAsync(async (req, res, next) => {
  const Orders = await Order.find(
    { idUser: req.user._id },
    { _id: false, __v: false, address: false, phone: false }
  );
  if (Orders.length === 0) {
...
function numberWithCommas(str) {
  return str.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".").concat('đ');
}
...
exports.Create_Product = catchAsync(async (req, res, next) => {
  ...
  if (req.body.priceProduct[req.body.priceProduct.length - 1] === 'đ') {
    priceProduct = req.body.priceProduct.slice(0, -1)
  }
  ...
  if (oldProduct.content.includes(titleCase(req.body.nameProduct))) {
  ...
  }else if(req.body.nameProduct.length>50){
    res.status(200).json({ status: `Product '${req.body.nameProduct}' is too long` });
  }else if(req.body.nameProduct===''){
    res.status(200).json({ status: `Product name is empty` });
  }else if(req.body.priceProduct===''){
    res.status(200).json({ status: `Price is empty` });
  }else if (isNaN(priceProduct.replaceAll('.', '')) || Number(priceProduct.replaceAll('.', ''))<0) {
    ...
    oldProduct.price.push(numberWithCommas(Number(priceProduct.replaceAll('.', ''))));
...
exports.Delete_Product = catchAsync(async (req, res, next) => {
  ...
  let deleteIndex = oldProduct.content.indexOf(req.body.nameProduct === 'TCF Coffee' ? req.body.nameProduct : titleCase(req.body.nameProduct));
  if (deleteIndex !== -1) {
...
exports.Update_Product = catchAsync(async (req, res, next) => {
  ...
  if (req.body.priceUpdate[req.body.priceUpdate.length - 1] === 'đ') {
  ...
  }else if(req.body.nameUpdate.length>50){
    res.status(200).json({ status: `Product '${req.body.nameUpdate}' is too long` });
  }else if(req.body.nameUpdate===''){
    res.status(200).json({ status: `Product name is empty` });
  }else if(req.body.priceUpdate===''){
    res.status(200).json({ status: `Price is empty` });
  }else if (isNaN(priceUpdate.replaceAll('.', '')) || Number(priceUpdate.replaceAll('.', ''))<0) {
    ...
    if (updateIndex != -1) {
      ...
      oldProduct.price[updateIndex] = numberWithCommas(Number(priceUpdate.replaceAll('.', '')));
