...
exports.Shopping_Cart_Handle = catchAsync(async (req, res) => {
  ...
  if (!checkUser) {
    ...
    const updateProduct = await Cart.updateOne({ idUser: req.user._id }, { products: [...req.body.products] });
...
exports.Get_Shopping_Cart_Data = (user_id) => {
  return new Promise(async (resolve, reject) => {
    resolve(await Cart.find({ idUser: user_id }, { _id: false, __v: false, idUser: false }));
  });
};
