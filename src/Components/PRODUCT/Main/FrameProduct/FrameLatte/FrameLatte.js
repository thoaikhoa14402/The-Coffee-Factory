...
function FrameLatte(props) {
  ...
  const [feeInt, setFeeInt] = useState(amount * props.price)

  const handleAdd = () => {
    setAmount(amount + 1)
    setFeeInt((amount + 1) * props.price)
  }
  const handleSub = () => {
    if (amount > 1) {
      setAmount(amount - 1)
      setFeeInt(feeInt - props.price)
    }
  }
  const handleAdd1 = () => {
    if (amount1 === 0 && amount2 === 0) setFeeInt(feeInt + 10)
    setAmount1(1)
    setAmount2(0)
  ...
  return <div className={style.FrameLatte}>
    ...
    <div className={style.information}>
      ...
      <div className={style.section2}>
        <p style={{ color: 'red' }} className={style.price}>{props.price}đ</p>
